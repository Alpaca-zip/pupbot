...
postureStabilization::postureStabilization() : _pnh("~")
{
  ...
  _pub_stabilization_variable =
    _nh.advertise<std_msgs::Float64MultiArray>("stabilization_variable", 10);
...
void postureStabilization::quatToRPY(
  const geometry_msgs::Quaternion quat, double & roll, double & pitch, double & yaw)
{
...
void postureStabilization::controlLoop()
{
  ...
  _e_LF_leg =
    -Y_OFFSET * tan(_roll_LPF.data / 180.0 * M_PI) + X_OFFSET * tan(_pitch_LPF.data / 180.0 * M_PI);
  _e_LR_leg =
    -Y_OFFSET * tan(_roll_LPF.data / 180.0 * M_PI) - X_OFFSET * tan(_pitch_LPF.data / 180.0 * M_PI);
  _e_RR_leg =
    Y_OFFSET * tan(_roll_LPF.data / 180.0 * M_PI) - X_OFFSET * tan(_pitch_LPF.data / 180.0 * M_PI);
  ...
  if (_posture_control_on) {
    _M_LF_leg = _M1_LF_leg + _P * (_e_LF_leg - _e1_LF_leg) + _I * _e_LF_leg +
                _D * ((_e_LF_leg - _e1_LF_leg) - (_e1_LF_leg - _e2_LF_leg));
    _M_LR_leg = _M1_LR_leg + _P * (_e_LR_leg - _e1_LR_leg) + _I * _e_LR_leg +
                _D * ((_e_LR_leg - _e1_LR_leg) - (_e1_LR_leg - _e2_LR_leg));
    _M_RR_leg = _M1_RR_leg + _P * (_e_RR_leg - _e1_RR_leg) + _I * _e_RR_leg +
                _D * ((_e_RR_leg - _e1_RR_leg) - (_e1_RR_leg - _e2_RR_leg));
    _M_RF_leg = _M1_RF_leg + _P * (_e_RF_leg - _e1_RF_leg) + _I * _e_RF_leg +
                _D * ((_e_RF_leg - _e1_RF_leg) - (_e1_RF_leg - _e2_RF_leg));
  } else {
    _M_LF_leg = _M1_LF_leg + 0 * (_e_LF_leg - _e1_LF_leg) + 0 * _e_LF_leg +
                0 * ((_e_LF_leg - _e1_LF_leg) - (_e1_LF_leg - _e2_LF_leg));
    _M_LR_leg = _M1_LR_leg + 0 * (_e_LR_leg - _e1_LR_leg) + 0 * _e_LR_leg +
                0 * ((_e_LR_leg - _e1_LR_leg) - (_e1_LR_leg - _e2_LR_leg));
    _M_RR_leg = _M1_RR_leg + 0 * (_e_RR_leg - _e1_RR_leg) + 0 * _e_RR_leg +
                0 * ((_e_RR_leg - _e1_RR_leg) - (_e1_RR_leg - _e2_RR_leg));
    _M_RF_leg = _M1_RF_leg + 0 * (_e_RF_leg - _e1_RF_leg) + 0 * _e_RF_leg +
                0 * ((_e_RF_leg - _e1_RF_leg) - (_e1_RF_leg - _e2_RF_leg));
  }
