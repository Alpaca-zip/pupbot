...
trotGait::trotGait() : _pnh("~")
{
  ...
  _sub_trot_foward_motion =
    _nh.subscribe("trot_foward_motion", 10, &trotGait::trotFowardMotionCallback, this);
  _sub_trot_turn_motion =
    _nh.subscribe("trot_turn_motion", 10, &trotGait::trotTurnMotionCallback, this);
...
void trotGait::trotTurnMotionCallback(const std_msgs::Float64 & turn) { _turn0 = turn.data; }

void trotGait::stopSignalCallback(const std_msgs::Bool & stop) { _stop_signal = stop.data; }
...
void trotGait::controlLoop()
{
  ...
  if (!_stop_signal) {
    for (int l = 0; l < 4; l++) {
      ...
      trot(
        rDirX() * _c[l], _l_inv[l][1] * rDirY() * _c[l], bool(l % 2) ^ bool(fmod(_c_inv[l], 2.0)));
